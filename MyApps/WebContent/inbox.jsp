...
<body>
	...
	<%
		...
		try{
			...
			while(rs.next()){
				...
				temp.setFrom(rs.getString("fromid"));
				temp.setTo((String)session.getAttribute("USER"));
				temp.setMessage(rs.getBytes("message"));
				temp.setEnKey(rs.getBytes("enkey"));
				temp.setMd(rs.getString("md"));
				temp.setMl(rs.getInt("ml"));
				temp.setDl(rs.getInt("dl"));
				temp.setKl(rs.getInt("kl"));
				temp.setTime(rs.getString("time"));
				temp.setSmsg(rs.getBytes("smsg"));
				f.add(temp); 
			}
	...
	<div class="card">
		<div class="card-header">
			  ...
			  <div class="card-body">
			    <h5 class="card-title"><td><%=f.get(i).getFrom() %></td></h5>
			    <p class="card-text"><%=Check.check(f.get(i).getFrom(), f.get(i).getMessage(), f.get(i).getEnKey(), f.get(i).getMd(), f.get(i).getMl())%></p>
		</div>
	</div>	
	<% } %>
